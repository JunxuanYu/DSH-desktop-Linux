...
Browser half of dynamic dual-half plugin packages. The host-side runner holds every definition's code in process memory and asks the open pages, over a `@deepseek-ai/cordis/request-run` event, whether to run one; this package answers that request, turns the definition into a live browser plugin, and turns a `@deepseek-ai/cordis/dynamic-retract` event back into a clean page.

## What it does

1. **Event subscription** — the four announcements are forwarded host cordis events, so this package consumes `@deepseek-ai/cordis/request-run`, `@deepseek-ai/cordis/request-run-resolved`, and `@deepseek-ai/cordis/dynamic-retract` through `ctx.remote.$on`, whose key set IS the api-remotes allowlist.
