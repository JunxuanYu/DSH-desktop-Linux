...
- `resolveRequestRun` 用作答页面的结论结束这次往返，并 emit `@deepseek-ai/cordis/request-run-resolved`，让其他每个页面撤下待作答的入口。首答即成；更晚的或未知的 request id 会被接受并忽略。命名了注册表已越过的版本的成功结论会被拒绝而非应用（`accepted: false`，请求仍处于挂起），因为作答的那个页面装载的是一个已不再存活的下发。失败的结论只会在 host 半正是由这次请求求值时才将它回退，因此某个页面装不上自己那一半，绝不会把其他页面正在使用的包停掉。
- `stop` 回退一次存活的下发——丢弃 handler、把 host 半 fiber dispose（资源释放）到完全停稳、emit `@deepseek-ai/cordis/dynamic-retract`——并让该定义仍然可运行。
...
本功能拥有四条转发事件，由本包在其 client-safe 的 [`./types`](src/types.ts) 子路径上声明，并由 [`@deepseek-ai/dsh-api-remotes`](../../api/remotes/README.md) 的白名单准许投递——正是这一点让浏览器能经 `ctx.remote.$on` 收到它们：`@deepseek-ai/cordis/request-run`（`{requestId, agentId, id, name, purpose}`——只有元数据，绝无代码）、`@deepseek-ai/cordis/request-run-resolved`（`{requestId, outcome}`）、`@deepseek-ai/cordis/dynamic-package`（`{id, name, rev}`），以及 `@deepseek-ai/cordis/dynamic-retract`（`{id, rev}`）。后两者是对称的一对运行状态播报：每次全新启动与每次停止都播，与该包有没有浏览器半无关。
