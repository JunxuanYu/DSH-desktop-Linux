...
export default defineConfig({
  ...
  test: {
    ...
    // apps/cli and apps/desktop only, not apps/*: apps/web/tests/*.e2e.ts needs
    // the built frontend dist and runs under vitest.web.config.ts (the test:web
    // job); apps/desktop e2e boots the built web runtime like the desktop app does.
    include: [
